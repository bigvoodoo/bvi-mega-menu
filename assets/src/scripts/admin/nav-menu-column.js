/**
 * BVI Mega Menu - Admin Column/Section item handler.
 *
 * Formats column items in the menu editor and handles the "Add to Menu" button.
 */

/* global wpNavMenu */

(function ($) {
	// save column data as JSON before form submit
	$('form#update-nav-menu').on('submit', function () {
		$(this)
			...
			.each(function () {
				...
				$(this)
					.find('.edit-menu-item-title-hidden')
					.val(JSON.stringify(data));
			});
	});

	/**
	 * Formats column boxes properly in the menu editor.
	 */
	const formatColumnFields = function () {
		...
		const hiddenTitle = $(
			'<input type="hidden" name="' +
				...
				'" class="edit-menu-item-title-hidden" value="" />'
		).val(titleField.val());
	...
	$('#menu-settings-column').on('click', function (e) {
		if ($(e.target).hasClass('submit-add-column-to-menu')) {
			...
			const url = $('#column-menu-item-url')
				.val()
				.replace('(optional)', '');
			const title = $('#column-menu-item-title')
				.val()
				.replace('(optional)', '');
	...
	// format column fields on page load
