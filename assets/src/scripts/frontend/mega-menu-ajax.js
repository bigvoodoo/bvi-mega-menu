...
function loadMenuAjax($item, themeLocation, home) {
  ...
  const url = '/ajax_mega_menu/' + encodeURIComponent(themeLocation) + '/' + id;
  const qs = jQuery.extend({}, $item.parents('ul.bvi-mega-menu-container').data());
...
function initContainers() {
  jQuery(SELECTORS.container).each(function () {
    ...
    const themeLocation = $container.data('theme_location') || $container.data('menu');
    ...
    $container.find(SELECTORS.depthZero).each(function () {
      ...
      el.addEventListener('mouseenter', function () {
        ...
        if (jQuery(el).children(SELECTORS.megaMenu).is(':visible') === false) {
          jQuery(`${SELECTORS.depthZero} > ${SELECTORS.megaMenu}`).stop(true, true).hide();
...
function initResizeHandler() {
  ...
  jQuery(window).on('resize', function () {
    ...
    jQuery(SELECTORS.mobileToggle).removeClass('open').next('ul').removeAttr('style');
  });
}

function isMobileView($container) {
  const toggle = ($container || jQuery(document)).find(SELECTORS.mobileToggle);
