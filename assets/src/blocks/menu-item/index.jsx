...
import { useBlockProps, useInnerBlocksProps, InnerBlocks, InspectorControls, RichText } from '@wordpress/block-editor';
import { PanelBody, TextControl, ToggleControl, Notice, Button, Spinner } from '@wordpress/components';
      ...
      Promise.all([
        ...
        .then(([general, topLevel]) => {
          ...
          const sorted = merged.sort((a, b) => {
            const aTitle = decodeEntities(a.title.rendered).toLowerCase();
            const bTitle = decodeEntities(b.title.rendered).toLowerCase();
      ...
      <div className="bvi-link-picker">
        <div className="bvi-link-picker__preview">
          <div className="bvi-link-picker__preview-info">
            <span className="bvi-link-picker__preview-title">{title || url}</span>
            <span className="bvi-link-picker__preview-url">{url}</span>
          </div>
    ...
    <div className="bvi-link-picker">
      ...
      {!isLoading && results.length > 0 && (
        <ul className="bvi-link-picker__results">
          {results.map((page) => (
            <li key={page.id}>
              <button type="button" className="bvi-link-picker__result" onClick={() => handleSelect(page)}>
                <span className="bvi-link-picker__result-title">{decodeEntities(page.title.rendered)}</span>
                {page.parent === 0 && (
                  <span className="bvi-link-picker__badge">{__('Top level', 'bvi-mega-menu')}</span>
                )}
              </button>
            </li>
          ))}
      ...
      {looksLikeUrl(searchVal.trim()) && searchVal.trim() && (
        <Button variant="secondary" size="small" onClick={handleCustomUrl}>
      ...
      <p className="components-base-control__help">
        {__('Leave empty to make the label a dropdown toggle only.', 'bvi-mega-menu')}
      </p>
    </div>
  );
}
...
registerBlockType(metadata.name, {
  edit: function Edit({ attributes, setAttributes, clientId }) {
    const { label, url, kind, id, type, openInNewTab, labelColor, panelWidth } = attributes;
    ...
    return (
      <>
        <InspectorControls>
          <PanelBody title={__('Link', 'bvi-mega-menu')} initialOpen={true}>
            <LinkPicker
              url={url}
              title={label || ''}
              id={id}
              ...
              <Notice status="info" isDismissible={false}>
                <p>
                  {sprintf(
                    ...
                    __('This page has %d child page(s).', 'bvi-mega-menu'),
                    childPages.length
                  )}
                </p>
                <Button variant="secondary" size="small" onClick={handleAddChildrenPanel}>
          ...
          <PanelBody title={__('Item Style', 'bvi-mega-menu')} initialOpen={false}>
            <TextControl
              label={__('Label Color', 'bvi-mega-menu')}
              value={labelColor}
              onChange={(value) => setAttributes({ labelColor: value })}
              help={__('CSS color. Overrides the mega menu default link color for this item.', 'bvi-mega-menu')}
            />
            <TextControl
              ...
              help={__(
                'CSS value with unit (e.g. 480px, 32rem, 100%). Controls this' + ' item\u2019s dropdown width.',
                'bvi-mega-menu'
              )}
            />
          </PanelBody>
