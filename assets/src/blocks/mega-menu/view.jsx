	...
	class MegaMenuBlock {
		...
		init() {
			this.breakpoint = parseInt(
				this.nav.dataset.mobileBreakpoint || '960',
				10
			);
			this.mobileMode = this.nav.dataset.mobileMode || 'none';
			this.trigger =
				this.nav.dataset.dropdownTrigger === 'click'
					? 'click'
					: 'hover';
			this.closeDelay = Math.max(
				0,
				parseInt(this.nav.dataset.closeDelay || '300', 10)
			);
			this.mobileLevels = parseInt(
				this.nav.dataset.mobileLevels || '1',
				10
			);
			this.spanParent = this.nav.dataset.dropdownSpanParent === 'true';
			this.mobileDropdownAlignment =
				this.nav.dataset.mobileDropdownAlignment || 'viewport';
			...
			if (this.spanParent) {
				...
				if (parent) {
					...
					this.nav.style.setProperty(
						'--bvi-mm-span-left',
						'-' + (navRect.left - parentRect.left) + 'px'
					);
					this.nav.style.setProperty(
						'--bvi-mm-span-width',
						parentRect.width + 'px'
		...
		bindDesktopHandlers() {
			this.nav
				.querySelectorAll(SELECTORS.itemHasPanel)
				.forEach((item) => {
					...
					item.addEventListener('mouseleave', () => {
						...
						if (
							this.trigger === 'hover' ||
							item.classList.contains(CLASSES.itemOpen)
						) {
							this.scheduleClose(item);
						}
					});
		...
		bindOutsideClick() {
			document.addEventListener('click', (event) => {
				...
				this.nav
					.querySelectorAll(
						SELECTORS.itemHasPanel + '.' + CLASSES.itemOpen
					)
		...
		bindEscKey() {
			document.addEventListener('keydown', (event) => {
				...
				if (this.nav.classList.contains(CLASSES.open)) {
					MegaMenuBlock.closeHamburger(this.nav);
					const hamburgerEl = this.nav.querySelector(
						SELECTORS.hamburger
					);
					if (hamburgerEl) {
						hamburgerEl.focus();
