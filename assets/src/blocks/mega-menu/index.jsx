...
registerBlockType(metadata.name, {
	edit: function Edit({ attributes, setAttributes }) {
		...
		const styleVars = {
			...
			'--bvi-mm-dropdown-bg-hover':
				dropdownBackgroundColorHover || undefined,
			'--bvi-mm-dropdown-color': dropdownTextColor || undefined,
			'--bvi-mm-dropdown-color-hover':
				dropdownTextColorHover || undefined,
			'--bvi-mm-submenu-bg': submenuBackgroundColor || undefined,
			'--bvi-mm-submenu-bg-hover':
				submenuBackgroundColorHover || undefined,
			...
			'--bvi-mm-hamburger-bg-hover':
				hamburgerBackgroundColorHover || undefined,
			'--bvi-mm-hamburger-bg-open':
				hamburgerBackgroundColorOpen || undefined,
			'--bvi-mm-mobile-bg': mobileMenuBackgroundColor || undefined,
			'--bvi-mm-mobile-link-bg':
				mobileMenuLinkBackgroundColor || undefined,
			'--bvi-mm-mobile-link-bg-hover':
				mobileMenuLinkBackgroundColorHover || undefined,
			'--bvi-mm-mobile-color': mobileMenuTextColor || undefined,
			'--bvi-mm-mobile-color-hover':
				mobileMenuTextColorHover || undefined,
		...
		const svgMedia = useSelect(
			(select) =>
				hamburgerSvgId ? select('core').getMedia(hamburgerSvgId) : null,
			[hamburgerSvgId]
		);
		...
		const hamburgerStyleHelp = (() => {
			if (hamburgerStyle === 'custom') {
				return __(
					'Let a child theme supply the icon via the' +
						' bvi_nav_hamburger_open_icon filter.',
					'bvi-mega-menu'
				);
			}
			if (hamburgerStyle === 'svg') {
				return __(
					'Upload an SVG to use as the icon. Still animates to an X' +
						' via CSS rotation on open.',
					'bvi-mega-menu'
				);
			}
			return __(
				'Default three-bar icon; transforms into an X when open.',
				'bvi-mega-menu'
			);
		})();
		...
		return (
			<>
				<InspectorControls group="styles">
					...
					{hasMobile && (
						<PanelBody
							title={__('Mobile Styling', 'bvi-mega-menu')}
							initialOpen={true}
						>
							<p
								style={{
							...
							<SelectControl
								...
								onChange={(value) =>
									setAttributes({ mobileFontWeight: value })
								}
							/>
							...
							>
								<ToggleGroupControlOption
									value="normal"
									label={__('Normal', 'bvi-mega-menu')}
								/>
								<ToggleGroupControlOption
									value="italic"
									label={__('Italic', 'bvi-mega-menu')}
								/>
							</ToggleGroupControl>
							...
							>
								...
								<ToggleGroupControlOption
									value="none"
									label="—"
									aria-label={__('None', 'bvi-mega-menu')}
									showTooltip
								/>
							</ToggleGroupControl>
							...
							>
								<ToggleGroupControlOption
									value="solid"
									label={__('Solid', 'bvi-mega-menu')}
								/>
								<ToggleGroupControlOption
									value="dashed"
									label={__('Dashed', 'bvi-mega-menu')}
								/>
								<ToggleGroupControlOption
									value="dotted"
									label={__('Dotted', 'bvi-mega-menu')}
								/>
								<ToggleGroupControlOption
									value="double"
									label={__('Double', 'bvi-mega-menu')}
								/>
							</ToggleGroupControl>
						</PanelBody>
					)}
				...
				<InspectorControls>
					<PanelBody
						title={__('Menu Source', 'bvi-mega-menu')}
						initialOpen={true}
					>
						{isLoadingMenus ? (
							<Spinner />
					...
					>
						...
						<UnitControl
							...
							units={UNITS_SPACING}
							onChange={(value) =>
								setAttributes({ dropdownItemGap: value || '' })
							}
						/>
						<UnitControl
							...
							units={UNITS_SPACING}
							onChange={(value) =>
								setAttributes({ popupItemGap: value || '' })
							}
						/>
					</PanelBody>

					<PanelBody
						title={__('Dropdown Behavior', 'bvi-mega-menu')}
						initialOpen={true}
					>
						<ToggleGroupControl
							...
							onChange={(value) =>
								setAttributes({ dropdownTrigger: value })
							}
							isBlock
							help={__(
								'How desktop dropdowns open. Mobile always uses tap to expand.',
								'bvi-mega-menu'
							)}
						>
							<ToggleGroupControlOption
								value="hover"
								label={__('Hover', 'bvi-mega-menu')}
							/>
							<ToggleGroupControlOption
								value="click"
								label={__('Click', 'bvi-mega-menu')}
							/>
						</ToggleGroupControl>
						...
						<ToggleGroupControl
							...
							onChange={(value) =>
								setAttributes({ dropdownPanelAlignment: value })
							}
							isBlock
							help={__(
								'Which edge of the menu item the dropdown panel aligns to.',
								'bvi-mega-menu'
							)}
						>
							<ToggleGroupControlOption
						...
						<ToggleControl
							...
							onChange={(value) =>
								setAttributes({ dropdownSpanParent: value })
							}
							help={__(
						...
						<ToggleControl
							...
							onChange={(value) =>
								setAttributes({ showDropdownArrow: value })
							}
							help={__(
								'Show the small arrow indicator on menu items that have a dropdown.',
								'bvi-mega-menu'
							)}
						/>
						<RangeControl
							...
							onChange={(value) =>
								setAttributes({ dropdownCloseDelay: value })
							}
							min={0}
							max={10000}
							step={1}
						...
						{hasMobile && (
							<>
								...
								<RangeControl
									...
									value={mobileLevels}
									onChange={(value) =>
										setAttributes({ mobileLevels: value })
									}
									min={0}
									max={5}
									step={1}
								...
								<UnitControl
									...
									help={__(
										'Padding inside the mobile nav container.',
										'bvi-mega-menu'
									)}
								/>
								...
								<SelectControl
									...
									onChange={(value) =>
										setAttributes({ hamburgerStyle: value })
									}
				...
				<nav {...blockProps}>
					{hasMobile && (
						...
						>
							<span
								className="bvi-mega-menu-hamburger-bar"
								aria-hidden="true"
							/>
							<span
								className="bvi-mega-menu-hamburger-bar"
						...
						{isUsingInnerBlocks ? (
							...
							>
								<Notice status="info" isDismissible={false}>
									{__(
										'Clear the Menu dropdown in block settings to compose' +
											' from inner blocks instead.',
										'bvi-mega-menu'
									)}
								</Notice>
							</Placeholder>
